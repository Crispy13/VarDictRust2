...
description: >
  Mandatory protocol for VarDict-rs Claude Code workflow infrastructure changes.
  ...
  descriptions, test tiers, workflow references, or replacing Codex
  assumptions with Claude Code-compatible behavior. Do NOT use for Rust porting, parity
...
Read every `.agent.md` under `.claude/agents/`:
...
Read every `.instructions.md` under `.claude/instructions/`:
...
Read every `.yml` under `.claude/workflows/`:
...
| Skill-facing | A Claude Code session follows one skill and invokes related skills directly |
...
- Agents: `.claude/agents/{kebab-case}.agent.md`
- Skills: `.claude/skills/{kebab-case}/SKILL.md`
- Instructions: `.claude/instructions/{kebab-case}.instructions.md`
...
1. Extract all agent `name:` values from `.claude/agents/*.agent.md`
2. Search all `.claude/` files for agent name strings — flag any that don't match a
   known agent name
3. Extract all skill directory names from `.claude/skills/*/`
4. Search all `.claude/` files for skill name strings — flag any that don't match a
   known skill
...
```bash
python -c "import yaml; yaml.safe_load(open('.claude/workflows/{file}.yml'))"
```
