...
The `faithful-port` skill hands off here after Phase 3 (structural review) for pipeline modules. In a skill-only Claude Code workflow, the current CLI session invokes this skill with a module name. This skill runs the verification, then either confirms parity or routes failures to `shard-diagnosis`.
...
| 3b (`e2e_sweep_somatic`) | Full tumor/normal pair x all regions x 14 somatic configs | `parity_e2e_sweep` (filter `wes_il_pair_sweep::`) | `tmp/sweep_fixtures/output/` (somatic entries) | `scripts/gen_e2e_sweep_golden.sh --somatic --tags wes_il_pair --force` | `VARDICT_E2E_SWEEP_SHARD=i/N`, `VARDICT_E2E_SWEEP_SOMATIC_CONFIG=<name>` | `cargo test --profile debug-release --test parity_e2e_sweep wes_il_pair_sweep:: -- --include-ignored --test-threads=1` |
