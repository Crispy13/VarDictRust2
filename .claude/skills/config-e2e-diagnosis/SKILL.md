...
- For any wrapper-driven `scripts/e2e_sweep_gate.sh` or `scripts/e2e_sweep_gate.py`
  run, the current Claude Code session has confirmed the chosen `--test-threads` count with the user
...
- **Phase 1: Evidence intake / full-scope refresh** — the current Claude Code session may run this phase directly with a routed red artifact. No subagent dispatch is required before the first pass.
...
**Execution ownership:** the current Claude Code session runs Phase 2 and Phase 3
...
This phase is completed by the current Claude Code session as part of the same diagnosis
...
- The current Claude Code session runs Phases 1, 2, 3, and 5 and produces the diagnosis
  report, the Phase 3 failing-test contract, and the final verification report.
- The current Claude Code session uses `mismatch-repair` for Phase 4 repairs.
- User-reviewed plan files replace subagent dispatch: write the plan under the
...
| Executor | Phases |
|----------|--------|
| Current Claude Code session | Phases 1, 2, 3, 5 |
| Current Claude Code session using `mismatch-repair` | Phase 4 |
| Current Claude Code session using `change-impact-review` | Terminal performance verdict |
