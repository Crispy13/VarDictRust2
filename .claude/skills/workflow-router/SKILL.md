...
The `e2e-config-diagnosis` route is the canonical full-scope final gate. User-approved diagnostic reruns may happen inside that workflow, but they do not replace the canonical full-scope route. The route uses Claude Code plan mode only before the combined Phase 2/3 diagnosis-handoff dispatch and before the repair dispatch. It is not inserted before mechanical verification reruns. After a proven Rust parity repair verifies, the route must read the repair diff and run `logic-parity-audit` for the touched Rust module or logic surface before Review Gate/change-impact-review and before accepting the full-scope rerun as the next canonical step; broad or ambiguous Rust diffs require a full touched-module audit. Infrastructure-only repairs, cache refreshes, provenance fixes, and harness repairs are exempt from this logic-audit checkpoint. After a parity repair verifies and the required logic audit passes, the route must treat the repair as `PERF_PENDING` until Review Gate/change-impact-review records a terminal non-pending verdict: `PERF_SAFE`, `PERF_RISK`, `PERF_REGRESSION`, or `PERF_REGRESSION_ACCEPTED_PARITY_REQUIRED`. `PERF_PENDING` is not silently durable; it expires on the next code change to the same module/surface or the next full-gate cycle, whichever comes first.
...
### MEDIUM Confidence
- **Action:** Confirm with user via `request_user_input` with a single question with the recommended option pre-selected.
- **Format:** One question with 2-3 options, recommended option marked.

### LOW Confidence
- **Action:** Present full option list via `request_user_input`, let user choose.
