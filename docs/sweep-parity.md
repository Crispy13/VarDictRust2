...
For config E2E sweep gates, `scripts/e2e_sweep_gate.py` also validates each staged cached Java TSV against the paired `*.chunks.json` `monolithic_md5` fingerprint before the cargo parity phase. Missing TSVs, missing fingerprints, incompatible backfilled sidecars, unreadable TSVs, and fingerprint mismatches are reported as cache-readiness warnings; those artifacts require a fresh fixture refresh and must not be routed as Rust repair evidence.
