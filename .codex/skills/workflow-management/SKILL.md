...
description: >
  Mandatory protocol for VarDict-rs Codex workflow infrastructure changes.
...
Read every `SKILL.md` under `.codex/skills/*/`:
...
| Skill-facing | A Codex session follows one skill and invokes related skills directly |
...
- Skills: `.codex/skills/{kebab-case}/SKILL.md`
...
2. Search all `.github/` files for agent name strings — flag any that don't match a
   known agent name
3. Extract all skill directory names from `.codex/skills/*/`
