...
agents: [Port Engineer, Parity Verifier, Review Gate, Java Analyst, agent, Explore]
---
...
2. **Risk Assessment** — If high-risk triggers met (LOC > 1,000, >2 cross-module deps, 2+ prior parity failures, marked HIGH_RISK): invoke Java Analyst first.
3. **Delegate to Port Engineer** — Write task brief to `/memories/session/task-brief-{module}.md` with module name, stage, parity traps, success criteria. If Java Analyst analysis exists, include its path. Dispatch Port Engineer with the task brief path.
...
5. **Review & Commit** — Route to Review Gate with impl report path + parity report path. If PERF_SAFE: invoke git-commit skill, update active plan, advance. If PERF_RISK: document and proceed. If PERF_REGRESSION: block and route back.
