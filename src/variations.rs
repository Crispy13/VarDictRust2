...
use crate::data::{Sclip, Variation, VariationMap, Variant, Vars};
...
pub fn strand_bias(forward_count: i32, reverse_count: i32) -> i32 {
	if forward_count + reverse_count <= 12 {
		return if forward_count * reverse_count > 0 { 2 } else { 0 };
	}
...
pub fn find_conseq(soft_clip: &mut Sclip, dir: i32) -> String {
	...
	if !sequence.is_empty() && sequence.len() >= ADSEED as usize {
		...
		if dir == 3 {
			...
			let seed = sequence_prefix(&sequence, ADSEED);
			let reversed_seed = String::from_utf8_lossy(&reverse_sequence(seed.as_bytes())).into_owned();
...
pub fn adj_cnt_with_reference(
	...
	if current_scope().conf.y {
		let ref_count = reference_var
			...
			.map_or_else(|| String::from("NA"), |reference| reference.vars_count.to_string());
		eprintln!(
			"    AdjCnt: '+' {} {} {} {} Ref: {}",
...
mod tests {
	...
	fn init_sclip() -> Sclip {
		...
		sclip.seq = Some(BTreeMap::from([
			(
				0,
				BTreeMap::from([(String::from("C"), Variation {
					mean_quality: 82.0,
					..Variation::default()
				})]),
			),
			(
				1,
				BTreeMap::from([(String::from("T"), Variation {
					mean_quality: 82.0,
					..Variation::default()
				})]),
			),
			(
				2,
				BTreeMap::from([(String::from("A"), Variation {
					mean_quality: 78.0,
					..Variation::default()
				})]),
			),
	...
	fn get_var_maybe_reads_all_supported_sources() {
		...
		assert!(get_var_maybe(&aligned, 7, VarsType::Varn, VarMaybeArg::Description("A")).is_some());
	}
	...
	fn join_ref_large_insertion_helpers_match_java_indexing() {
		...
		assert_eq!(join_ref_for_3_lgins(&reference, 1, 4, 1, "TTAA", "T"), "ATAA");
	}
	...
	fn get_var_maybe_returns_none_for_missing_entries() {
		...
		assert!(get_var_maybe_from_vars(&vars, VarsType::Varn, VarMaybeArg::Description("A")).is_none());
	}
	...
	fn find_conseq_returns_empty_when_consensus_is_weak() {
		...
		sclip.seq = Some(BTreeMap::from([
			(
				0,
				BTreeMap::from([
					(String::from("A"), Variation {
						mean_quality: 10.0,
						..Variation::default()
					}),
					(String::from("C"), Variation {
						mean_quality: 12.0,
						..Variation::default()
					}),
				]),
	...
	fn get_var_maybe_ref_ignores_extra_arg() {
		...
		assert!(get_var_maybe_from_vars(&vars, VarsType::Ref, VarMaybeArg::Description("ignored")).is_some());
	}
	...
	fn join_ref_for_3_lgins_can_inject_sequence_in_middle() {
		...
		assert_eq!(join_ref_for_3_lgins(&reference, 5, 8, 2, "GGCC", "G"), "ACGC");
	}
