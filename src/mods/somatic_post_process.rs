...
fn calling_for_one_sample(
    ...
    let ref_var_owned = variants.reference_variant.clone();
    for &idx in variants.variants.iter() {
        let mut variant = variants.arena[idx].clone();
...
fn print_variations_from_first_sample(
    ...
    while processed < v1.variants.len() {
        let current = v1.arena[v1.variants[processed]].clone();
        let current_type = current.var_type();
        let v1_ref_owned = v1.reference_variant.clone();
        ...
        if let Some(list_pos) = v2
            .variants
            .iter()
            .position(|&ai| v2.arena[ai].description_string == nt)
        {
            let v2_ref_owned = v2.reference_variant.clone();
            let v2ai = v2.variants[list_pos];
            ...
            if vref.vartype != SNV && (nt.len() > 10 || MINUS_NUM_NUM.is_match(&nt)) {
                ...
                // Insert a placeholder into v2's varn map so combine_analysis can look it up
                let v2_placeholder_idx = v2.arena.len();
                v2.arena.push(v2nt.clone());
                v2.var_description_string_to_variants
                    .insert(nt.clone(), v2_placeholder_idx);
    ...
    for &v2ai in v2.variants.iter() {
        let mut v2var = v2.arena[v2ai].clone();
            ...
            get_var_maybe_from_vars(v1, VarsType::Varn, VarMaybeArg::Description(&nt))
        {
            ...
            let fwd = v1
                .reference_variant
                .as_ref()
                .map_or(0, |rv| rv.vars_count_on_forward);
            let rev = v1
                .reference_variant
                .as_ref()
                .map_or(0, |rv| rv.vars_count_on_reverse);
            let genotype = if let Some(v1var) = v1var {
                v1var.genotype.clone().unwrap_or_else(|| String::from("0"))
            } else if let Some(v1ref) = v1.reference_variant.as_ref() {
...
fn print_variations_from_second_sample(
    ...
    for &v2ai in v2.variants.iter() {
        ...
        // Get or create a placeholder arena slot in v1 for the given description
        let v1nt_idx = if let Some(&existing) = v1
            ...
            .get(&description_string)
        {
            ...
            let i = v1.arena.len();
            v1.arena.push(Variant::default());
            v1.var_description_string_to_variants
                .insert(description_string.clone(), i);
            i
        };
        v1.arena[v1nt_idx].position_coverage = 0;

        if v2
            .var_description_string_to_variants
            .get(&description_string)
            .is_some_and(|&ai| v2.arena[ai].position_coverage < conf.minr + 3)
            ...
            && (description_string.len() > 10 || MINUS_NUM_NUM.is_match(&description_string))
        {
            let v2_desc_owned = v2
                .var_description_string_to_variants
                .get(&description_string)
                .map(|&ai| v2.arena[ai].clone())
                .expect("BAM2 variant description must exist");
            let mut v1nt_val = v1.arena[v1nt_idx].clone();
            let combine_data = combine_analysis(
                &v2_desc_owned,
                &mut v1nt_val,
                &region.chr,
            ...
            // Write back the mutated v1nt_val into the arena slot
            v1.arena[v1nt_idx] = v1nt_val;
            *max_read_length = combine_data.max_read_length;
